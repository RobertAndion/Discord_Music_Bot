...
class music(commands.Cog):
    ...
    @commands.command(name = 'play', description=".play {song name} to play a song, will connect the bot.") #Allows for a song to be played, removed need of join
    ...
    async def unshuffle(self,ctx):
        ...
        if player.is_playing:
            try:
                ...
                #can use channel id to make sure command sender is in same voice chat###
            except Exception as error:
                print(error)
                #NEED CLEAR COMMAND AND DESCRIPTIONS
        else:
